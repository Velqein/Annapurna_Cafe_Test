...
/* ─── Food SVG illustrations (bowl art) ─────────────────────────────────────── */
...
/* ─── Food Card (showcase only — no add-to-cart) ────────────────────────────── */
    ...
    >
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: C.green, fontFamily: "'Poppins',sans-serif" }}>
            ₹{item.price}
          </span>
          {/* veg indicator dot */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <div style={{
              ...
              border: `1.5px solid ${item.type === 'noodles' && item.tag === 'NON-VEG' ? '#C0392B' : '#27AE60'}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <div style={{
                width: 5, height: 5, borderRadius: '50%',
                background: item.tag === 'NON-VEG' ? '#C0392B' : '#27AE60',
              }}/>
            </div>
          </div>
        </div>
...
/* ─── Menu Section ───────────────────────────────────────────────────────────── */
function MenuSection({ data, delay = 0 }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay }} style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14, paddingRight: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingLeft: 20, paddingRight: 20, paddingBottom: 6 }}>
        {data.items.map((item, i) => (
          <FoodCard key={i} item={item} delay={delay + i * 0.07}/>
        ))}
      </div>
    </motion.div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#c8c4bc', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px' }}>
      <IPhoneFrame>
        ...
        {/* ── Background blobs & decorations (fixed, non-scrolling) ── */}
        ...
        {/* ── Scrollable content ── */}
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, scrollbarWidth: 'none' }}>
          ...
          <motion.div {...fadeUp(0.25)} style={{ margin: '0 20px 16px' }}>
            <div style={{
              ...
              <div style={{ position: 'absolute', top: 12, right: 90, opacity: 0.4 }}>
                <Dots n={3} opacity={0.3}/>
              </div>

              {/* Left text */}
              <div style={{ zIndex: 2, flex: 1 }}>
                <div style={{
                  background: C.blue, borderRadius: 8, padding: '3px 10px',
                ...
                <div style={{ fontFamily: "'Caveat',cursive", fontSize: 22, fontWeight: 700, color: C.green, lineHeight: 1.1, marginBottom: 10 }}>
                  Schezwan<br/>Noodles
                </div>
                <div style={{ fontFamily: "'Bungee',sans-serif", fontSize: 20, color: C.blue }}>₹169</div>
              </div>
              ...
              {categories.map(({ id, label }, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 9.5, fontWeight: 600, color: isActive?'white':C.blue, textAlign: 'center', lineHeight: 1.2 }}>
                      {label}
                    </span>
                  </motion.div>
                )
        ...
        <div style={{
          ...
          {[
            ...
            <div key={i} onClick={item.onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: item.onClick ? 'pointer' : 'default' }}>
              {i === 0 && <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M3 10 L11 3 L19 10 V19 H14 V14 H8 V19 H3Z" stroke={C.blue} strokeWidth="1.8" fill="none" strokeLinejoin="round"/></svg>}
              {i === 1 && <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="10" cy="10" r="6.5" stroke={C.blue} strokeWidth="1.8"/><path d="M15 15 L19 19" stroke={C.blue} strokeWidth="1.8" strokeLinecap="round"/></svg>}
              {i === 3 && <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M3 3 L7 7 M15 15 L19 19 M7 7 L12 5 L17 7 L19 12 L17 17 L12 19 L7 17 L5 12Z" stroke={C.blue} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><circle cx="11" cy="11" r="2" stroke={C.blue} strokeWidth="1.5"/></svg>}
              {i === 4 && <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="8" r="4" stroke={C.blue} strokeWidth="1.8"/><path d="M3 19 C3 15.7 6.6 13 11 13 C15.4 13 19 15.7 19 19" stroke={C.blue} strokeWidth="1.8" strokeLinecap="round"/></svg>}
              <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 9, fontWeight: 500, color: C.blue, opacity: 0.6 }}>{item.label}</span>
            </div>
          ))}
        </div>
